...
function startMove2(obj,json,fnEnd){
	clearInterval(obj.timer);
	obj.timer = setInterval(function(){
		doMove2(obj,json,fnEnd);
	},30);
}
function doMove2(obj,json,fnEnd){
	var iCur = 0;
	var attr = null;
	var bStop = true;
	for(attr in json){
		if(attr=='opacity'){
			if(parseInt(100*getStyle2(obj,attr))==0){
				iCur = parseInt(100*getStyle2(obj,attr));
			}
			else{
				iCur = parseInt(100*getStyle2(obj,attr)) || 100;
			}
		}
		else{
			iCur = parseInt(getStyle2(obj,attr)) || 0;
		}
